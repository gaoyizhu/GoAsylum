...
export function createInitialGameState(): GameState {
  return {
    ...
    koPosition: null,
    deadStones: [],
    showRealColors: false, // 失忆症模式默认不显示真实颜色
  };
}
...
export function makePass(state: GameState): GameState {
  ...
  if (newConsecutivePasses >= 2) {
    ...
    // 中国规则：黑棋需要至少 44 个子才能获胜（贴 3.5 目）
    // 9×9 = 81 个交叉点，黑棋需要 44，白棋需要 38 即可获胜
    let winner: 'black' | 'white' | 'draw';
    if (blackScore >= 44) {
      winner = 'black';
    } else if (whiteScore >= 38) {
      winner = 'white';
    } else {
      winner = 'draw';
    }
...
export function judgeGame(state: GameState): GameState {
  ...
  // 中国规则：9×9 棋盘，黑棋需要至少 44 个交叉点才能获胜
  let winner: 'black' | 'white' | 'draw';
  if (blackScore >= 44) {
    winner = 'black';
  } else {
